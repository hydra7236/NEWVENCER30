...
class Media {
  ...
  onResize({ screen, viewport }: any = {}) {
    ...
    if (this.imageScale) {
      this.plane.scale.x *= this.imageScale;
      this.plane.scale.y *= this.imageScale;
    }
...
class GalleryApp {
  ...
  constructor(container: HTMLElement, { items, bend, textColor = '#ffffff', borderRadius = 0, font = 'bold 30px Figtree', scrollSpeed = 2, scrollEase = 0.05, autoScroll = false, autoScrollInterval = 5000, imageScale = 1 }: any = {}) {
  ...
  createMedias(items: any[], bend = 1, textColor: string, borderRadius: number, font: string) {
    ...
    this.medias = this.mediasImages.map((data, index) => {
      ...
      media.imageScale = this.imageScale;
      media.onResize(); // trigger recalculation with new imageScale
      return media;
    });
  }
...
export default function CircularGallery({
  ...
  useEffect(() => {
    ...
    const app = new GalleryApp(containerRef.current, { items, bend, textColor, borderRadius, font, scrollSpeed, scrollEase, autoScroll, autoScrollInterval, imageScale });
    return () => {
      app.destroy();
    };
  }, [items, bend, textColor, borderRadius, font, scrollSpeed, scrollEase, autoScroll, autoScrollInterval, imageScale]);
